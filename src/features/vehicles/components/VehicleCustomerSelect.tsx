...
export function VehicleCustomerSelect({
  ...
  const {
    ...
    {
      organizationId,
      search: shouldSearch ? normalizedSearch : undefined,
      isActive: true,
    },
    {
      skip: !shouldSearch,
  ...
  const { data: defaultCustomers = [] } = useGetCustomersQuery(
    {
      organizationId,
      isActive: true,
    },
  ...
  const defaultCustomer = useMemo(
    () =>
      defaultCustomers.find((customer) => customer.id === defaultValue) ?? null,
    [defaultCustomers, defaultValue],
  );

  const currentCustomer =
    selectedCustomer ?? (!selectionCleared ? defaultCustomer : null);

  const results = useMemo(() => customers.slice(0, 10), [customers]);
    ...
    <div className="space-y-2">
          ...
          <div className="flex h-11 items-center justify-between rounded-lg border border-zinc-300 bg-white px-3">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate text-sm font-medium text-zinc-900">
                {getCustomerDisplayName(currentCustomer)}
              </span>
            </div>
      ...
      {!currentCustomer ? (
        <p className="text-xs leading-5 text-zinc-500">
          Type at least 2 characters to search active customers. Leave blank for
